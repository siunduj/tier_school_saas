...
class TeacherApiController extends Controller
{
    ...
    public function subjects(Request $request)
    {
        ...
        try {
            ...
            $subjects = $subjects->with('subject:id,name,type', 'class_section');

            $section_ids = explode(",", urldecode($request->class_section_id));
            // dd($section_ids);
            $subjects = $subjects->whereIn('class_section_id', $section_ids);
            ...
            if (count($section_ids) > 1) {
                // Group the subjects by their 'subject_with_name' value
                $groupedBySubjectName = $subjects->groupBy('subject_with_name');
                
                // Filter to only keep subjects that appear more than once (common subjects)
                $commonSubjects = $groupedBySubjectName->filter(function ($group) {
                    return $group->count() > 1;
                });
            
                // Flatten the result to get a simple list of common subjects
                $commonSubjectsList = $commonSubjects->flatten(1);
            
                // Remove duplicate records based on 'subject_with_name' and reset the array keys
                $commonSubjectsList = $commonSubjectsList->unique('subject_with_name')->values();
