    ...
    public function login(Request $request)
    {
        ...
        if ($request->code ) {
            ...
            if (Auth::guard('web')->attempt([
                ...
                if ($data) {
                    if (( $data->two_factor_secret == null || $data->two_factor_expires_at == null ) && $data->two_factor_enabled == 1 && !Auth::user()->hasRole('Teacher') && $request->email != 'demo@school.com') {
                        $twoFACode = $this->generate2FACode();
                        $settings = $this->cache->getSystemSettings();
                        $user = Auth::user();

                        DB::table('users')->where('email',$user->email)->update(['two_factor_secret' => $twoFACode]);
            ...
            if (Auth::guard('web')->attempt([
                ...
                if ($data) {
                    if (( $data->two_factor_secret == null || $data->two_factor_expires_at == null ) && $data->two_factor_enabled == 1 && !Auth::user()->hasRole('Teacher') && $request->email != 'superadmin@gmail.com' ) {
